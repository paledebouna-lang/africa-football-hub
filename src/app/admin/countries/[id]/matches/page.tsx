...
import { saveNationalTeamMatch, deleteNationalTeamMatch } from "../../../actions";
...
export default async function AdminNationalTeamMatchesPage({
  params,
}: {
  ...
  const [country, matches, opponents, competitions, seasons] = await Promise.all([
    prisma.country.findUnique({ where: { id } }),
    prisma.nationalTeamMatch.findMany({
      where: { OR: [{ homeCountryId: id }, { awayCountryId: id }] },
      orderBy: { date: "desc" },
      include: {
        competition: true,
        homeCountry: true,
    ...
    prisma.country.findMany({ where: { id: { not: id } }, orderBy: { nameFr: "asc" } }),
  ...
  const fields: Field[] = [
    ...
    { kind: "number", name: "ownScore", label: `Buts de ${country.nameFr}` },
    { kind: "number", name: "opponentScore", label: "Buts de l'adversaire" },
    {
      ...
      defaultValue: "SENIOR",
      hint: "Détermine quels joueurs sélectionnés sont proposés sur la feuille de match.",
    },
    { kind: "number", name: "matchday", label: "Journée" },
    { kind: "text", name: "venue", label: "Stade" },
    { kind: "hidden", name: "countryId", label: "", defaultValue: country.id },
  ];

  return (
    <AdminShell
      title={`Matchs de la sélection — ${country.nameFr}`}
      action={
        <Link
    ...
    >
      <div className="space-y-8">
                  ...
                  {matches.map((match) => (
                    <tr key={match.id}>
                      ...
                      <td className="px-4 py-3">
                        {match.homeCountry.nameFr} — {match.awayCountry.nameFr}
                      </td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center justify-end gap-4">
                          <Link
                            href={`/admin/countries/${country.id}/matches/${match.id}`}
                          ...
                          <form action={deleteNationalTeamMatch}>
        ...
        <section className="rounded-lg border border-border bg-surface p-6">
          ...
          <AdminForm
            action={saveNationalTeamMatch}
            cancelHref={`/admin/countries/${country.id}`}
